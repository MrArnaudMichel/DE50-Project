...
Le plugin expose **huit cas d'utilisation**, tous déclenchés par l'ingénieur système depuis le menu contextuel de Rhapsody ou en ligne de commande. Ils sont regroupés en trois grandes fonctionnalités : **initialisation**, **analyse SVN**, et **visualisation**.
...
| **Acteurs concernés** | Ingénieur système |
...
|---|---|
| **Nom** | SVN Calculate |
| **Description** | Calcule le score d'importance normalisé de chaque `«stakeholder»` du modèle en appliquant les équations SVN (Cameron 2007 / INCOSE 2018). |
| **Acteurs concernés** | Ingénieur système |
...
| **Traitement** | `CalculationService.calculateImportance()` — parcourt récursivement le modèle, construit le graphe des arcs, recherche les *value loops* par DFS depuis le nœud `«system»`, puis calcule l'importance relative de chaque stakeholder (Équation 2, Cameron 2007). En l'absence de nœud `«system»`, bascule sur un calcul simplifié par somme des arcs. |
...
**Matrice de score des arcs (INCOSE 2018, Figure 3) :**
...
| **Traitement** | `DiagramService.updateArcLabels()` — parcourt tous les arcs `«valuearc»` du modèle et met à jour leur `displayName` avec le format `B:<benefitRanking> | S:<supplyImportance>`. En cas d'échec de `setDisplayName`, tente une mise à jour via les propriétés graphiques des éléments de diagramme. |
| **Sorties** | Les arcs affichent leurs pondérations dans le diagramme (ex. : `B:MUST_BE | S:HIGH`). |
